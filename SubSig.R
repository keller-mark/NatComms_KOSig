...
BootstrapSig_quantile_final_subs_bg <- function(muttype_profile,parent_muttype_profile,cnum, subnum, bsnum,h,w,outputfilename){
  ...
  if(length(which((parentGenome_percentage<=bootstrapsubClone_threshold)))==96 | (clonecentroidDist<=cloneFrobeniusDist_threshold)){
    ...
    while(p<=subclonesum & stop_flag==0){
      ...
      if(max(apply((bootstrapsubClone_threshold-reconstructclone_all_percentage), 2, min))<0 & min(bs_reclonecentroidDist_all)>cloneFrobeniusDist_threshold){
        ...
        if(reconstructDist_distinct_num>5){
          ...
          #knockoutBasis <- read.table("MSH6_knockout_96basis_quantile500_final2.txt",sep = "\t",header = F, as.is = T)
          #parentBasis <- read.table("MSH6_parent_96basis_quantile500_final2.txt",sep = "\t",header = F, as.is = T)
          plotbasis(knockoutBasis,parentchildGenome$MutationType,1,paste0("rank=",2),paste0(outputfilename,"_knockout_96basis_quantile500_final2"),2,8)
          plotbasis(parentBasis,parentchildGenome$MutationType,1,paste0("rank=",2),paste0(outputfilename,"_parent_96basis_quantile500_final2"),2,8)
...
BootstrapNum <- 630
mean_background <- rowSums(hap1_parentchild96[,c(2,10,18,26,34,42,50,58,66)])

BootstrapDiff_final_meanclone(hap1_parentchild96,CloneNum,SubcloneNum,BootstrapNum,3,4,"bootstrap_sample_subs63000_1")

################################################
# Extract knockout signature using bootstraping
################################################
start_col <- 2
end_col <- dim(hap1_parentchild96)[2]-1
...
parentclones <- hap1_parentchild96[,c(2,10,18,26,34,42,50,58,66)]
for(i in 1:dim(knockoutlist)[1]){
  print(i)
  knockoutgene <- knockoutlist[i,1]
  parentchildGenome <- hap1_parentchild96[,c(1,(start_col+TotalcloneNum*(i-1)):(1+TotalcloneNum*i))]
  BootstrapSig_quantile_final_subs_bg(parentchildGenome,parentclones,CloneNum,SubcloneNum,BootstrapNum,3,10,knockoutgene)
}
