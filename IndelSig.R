...
BootstrapSig_quantile_final_indel_bg <- function(muttype_profile,parent_muttype_profile,cnum, subnum, bsnum,h,w,outputfilename){
  ...
  # Remove outliers of subclones, according to the mutation number using absolute deviation around median 
  ...
  subcloneFrobeniusDist <- apply(sel_subclones_percentage_all_diff,2, function(x) norm(as.matrix(x),"f"))
  subcloneFrobeniusDist_threshold <- quantile(subcloneFrobeniusDist, probs = 0.80)
  selected_bootstrapsubClone=sel_subclones_all[,which(subcloneFrobeniusDist<=subcloneFrobeniusDist_threshold)]
  
  bootstrapsubClone_threshold <- apply(sel_subclones_percentage_all, 1, quantile, probs=0.80)
    ...
    while(p<=subclonesum & stop_flag==0){
      ...
      if(max(apply((bootstrapsubClone_threshold-reconstructclone_all_percentage), 2, min))<0 & min(bs_reclonecentroidDist_all)>cloneFrobeniusDist_threshold){
        ...
        if(reconstructDist_distinct_num>5){
          ...
          indel5type <- c("Complex","Insertion","Insertion","Mh-mediated","Mh-mediated","None","Rep-mediated","Rep-mediated")
          
          indel_mypalette <- c("#0072B2", "#D55E00","#F0E442","#CC79A7","#009E73")
          indel_positions <- c("Ins_1","Ins_2>=","Microhomology-mediated_2","Microhomology-mediated_3>=","Repeat-mediated_1","Repeat-mediated_2>=","None","Complex") 
          indel_labels <- c("Insertion=1bp","Insertion>=2bp","Mh-mediated Del.=2bp","Mh-mediated Del.>=3bp","Rep-mediated Del.=1bp","Rep-mediated Del.>=2bp","Other Del.","Complex") 
          
          # plotbasis_indel(knockoutExposure,parentchildGenome$indelsubtype,indel5type,1,paste0("rank=",2),paste0(knockoutlist[i,1],"_9basis"),5,5)
          plotbasis_indel(knockoutBasis,parentchildGenome$indelsubtype,indel5type,indel_mypalette,indel_positions,indel_labels,1,paste0("rank=",2),paste0(outputfilename,"_knockout_9basis_mypalette"),h,w)
          plotbasis_indel(parentBasis,parentchildGenome$indelsubtype,indel5type,indel_mypalette,indel_positions,indel_labels,1,paste0("rank=",2),paste0(outputfilename,"_parent_9basis_mypalette"),h,w)
...
BootstrapDiff_final_meanclone(hap1_parentchild,CloneNum,SubcloneNum,BootstrapNum,3,4,"bootstrap_meanclone_indels63000")

################################################
# Extract knockout signature using bootstraping
################################################

start_col <- 2
end_col <- dim(hap1_parentchild)[2]-1
CloneNum <- 1
SubcloneNum <- 7
TotalcloneNum <- SubcloneNum + CloneNum
BootstrapNum <- 100
...
for(i in 1:dim(knockoutlist)[1]){
  ...
  BootstrapSig_quantile_final_indel_bg(parentchildGenome,parentclones,CloneNum,SubcloneNum,BootstrapNum,3,4,paste0("indelsig_final_",knockoutlist[i,1]))
